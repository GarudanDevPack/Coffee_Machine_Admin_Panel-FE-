...
import { addNotificationType, updateNotificationTypeById } from '../../actions/types/notificationTypeAction'

const AddNotificationTypeModal = ({ visible, onClose,editData,addOREdit  }) => {
...
const resetForm = () => {
  ...
  useEffect(() => {
      ...
      if (editData) {
        ...
        setNotificationTypename(editData.name || '')
  ...
  const handleClientChange = (event) => {
   ...
   const handleNotificationTypeName = (event) => {
    setNotificationTypename(event.target.value)
  }
...
dataToUpdate = {
  ...
  is_active: isActive
      ...
      try {
        let response = {}
        if (!editData) {
          response = await dispatch(addNotificationType(dataToSend))
        } else {
          console.log(data)
          response = await dispatch(updateNotificationTypeById(dataToUpdate))
        }
        if (response) {
          Swal.fire({
            title: 'Saved!',
            ...
            <CRow>
              ...
              <CCol className="mt-2" md={12}>
                <CFormInput
                  ...
                  onChange={handleNotificationTypeName}
                />
              </CCol>
...
export default AddNotificationTypeModal
